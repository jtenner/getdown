...
const MarkdownBlock = memo(function MarkdownBlock({ block }: { block: MarkdownBlockNode }): ReactElement {
  ...
  switch (block.kind) {
    ...
    case "code": {
      ...
      if (CodeBlock) return <CodeBlock node={block} code={block.text} language={block.language} />;
      return (
        <pre>
    ...
    case "list": {
      ...
      return ListComponent ? <ListComponent node={block} ordered={block.ordered} start={start}>{children}</ListComponent> : <List start={start}>{children}</List>;
    }
    case "table":
...
function renderListItem(item: ListItem, index: number, renderers: RendererContextValue): ReactElement {
  ...
  return ListItemComponent ? <ListItemComponent key={index} node={item} index={index} task={item.task}>{children}</ListItemComponent> : <li key={index}>{children}</li>;
}
...
function renderTable(block: TableBlock, renderers: RendererContextValue): ReactElement {
  ...
  const headerCells = block.header.map((cell, index) => {
    ...
    return HeaderCell ? (
      <HeaderCell key={index} node={cell} table={block} columnIndex={index} align={align}>{children}</HeaderCell>
    ) : (
  ...
  const bodyRows = block.rows.map((row, rowIndex) => {
    const cells = row.map((cell, cellIndex) => {
      ...
      const children = renderInlines(cell.children, renderers);
      return BodyCell ? (
        <BodyCell key={cellIndex} node={cell} table={block} rowIndex={rowIndex} columnIndex={cellIndex} align={align}>{children}</BodyCell>
      ) : (
...
function renderInline(node: InlineNode, key: number, renderers: RendererContextValue): ReactNode {
  switch (node.kind) {
    ...
    case "link": {
      ...
      const children = renderInlines(node.children, renderers);
      return Link ? <Link key={key} node={node} href={href} title={node.title}>{children}</Link> : <a key={key} href={href} title={node.title}>{children}</a>;
    }
    case "image": {
      const Image = renderers.onImageComponent;
      const src = renderers.sanitizeImageSrc(node.src, node);
      return Image ? <Image key={key} node={node} src={src} alt={node.alt} title={node.title} /> : <img key={key} src={src} alt={node.alt} title={node.title} />;
    }
  }
}
